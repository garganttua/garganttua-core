 ...
 *
 * <p>
 * IConditionBuilder provides a type-safe, chainable API for creating complex boolean conditions
 * using boolean algebra operators (AND, OR, XOR, NOT, NAND, NOR) and custom predicates. The builder
 * pattern enables readable, declarative condition definitions without nested constructor calls.
 * </p>
 *
 * <h2>Core Features</h2>
 * <ul>
 *   <li><b>Boolean Operators</b> - Combine conditions with and(), or(), xor(), not(), nand(), nor()</li>
 *   <li><b>Null Checks</b> - isNull(), isNotNull() for object existence validation</li>
 *   <li><b>Equality</b> - equals(), notEquals() for value comparison</li>
 *   <li><b>Custom Predicates</b> - custom() for arbitrary boolean logic with property extraction</li>
 ...
 *
 * IObjectSupplier<User> userSupplier = () -> getCurrentUser();
 *
 ...
 *     custom(userSupplier, User::isSuspended, suspended -> suspended)
 * ).build();
 *
 * // XOR: premium OR trial, but not both
 * ICondition hasExactlyOneAccess = xor(
 *     custom(userSupplier, User::isPremium, p -> p),
 *     custom(userSupplier, User::isTrialActive, t -> t)
 * ).build();
 * }</pre>
 *
 * <h2>Integration with Runtime</h2>
 * <p>
 * Conditions built with IConditionBuilder can be used in runtime workflows to control
 * step execution:
 * </p>
 * <pre>{@code
 * @RuntimeDefinition
 ...
 *         return and(
 *             isNotNull(() -> order),
 *             custom(() -> order, Order::getStatus, status -> "PENDING".equals(status))
 *         ).build();
 *     }
 * }
 * }</pre>
 ...
 */
public interface IConditionBuilder extends IBuilder<ICondition>{
