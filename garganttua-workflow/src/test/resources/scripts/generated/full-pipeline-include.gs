...
# Generated: 2026-05-26T18:18:21.294163295Z
...
_fetch_api_fetcher_ref <- include("/tmp/junit3603125482533174588/fetch-api.gs")
...
data <- @rawData
strict <- true
_validation_data_validator_ref <- include("/tmp/junit3603125482533174588/validate-data.gs")
...
inputData <- @validated
format <- @targetFormat
_transform_data_transformer_ref <- include("/tmp/junit3603125482533174588/transform-data.gs")
...
# Stage: statistics
values <- @transformed
_statistics_stats_calculator_ref <- include("/tmp/junit3603125482533174588/calculate-stats.gs")
...
_finalize_finalizer_ref <- include("/tmp/junit3603125482533174588/finalize-output.gs")
