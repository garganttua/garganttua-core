...
# Generated: 2026-05-29T09:34:02.416519500Z
...
_fetch_api_fetcher_ref <- include("/tmp/junit10273921780874418621/fetch-api.gs")
...
data <- @rawData
strict <- true
_validation_data_validator_ref <- include("/tmp/junit10273921780874418621/validate-data.gs")
...
inputData <- @validated
format <- @targetFormat
_transform_data_transformer_ref <- include("/tmp/junit10273921780874418621/transform-data.gs")
...
# Stage: statistics
values <- @transformed
_statistics_stats_calculator_ref <- include("/tmp/junit10273921780874418621/calculate-stats.gs")
...
_finalize_finalizer_ref <- include("/tmp/junit10273921780874418621/finalize-output.gs")
