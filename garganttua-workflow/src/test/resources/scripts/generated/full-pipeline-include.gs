...
# Generated: 2026-05-27T15:52:45.581421774Z
...
_fetch_api_fetcher_ref <- include("/tmp/junit13225142256682985248/fetch-api.gs")
...
data <- @rawData
strict <- true
_validation_data_validator_ref <- include("/tmp/junit13225142256682985248/validate-data.gs")
...
inputData <- @validated
format <- @targetFormat
_transform_data_transformer_ref <- include("/tmp/junit13225142256682985248/transform-data.gs")
...
# Stage: statistics
values <- @transformed
_statistics_stats_calculator_ref <- include("/tmp/junit13225142256682985248/calculate-stats.gs")
...
_finalize_finalizer_ref <- include("/tmp/junit13225142256682985248/finalize-output.gs")
