...
# Generated: 2026-05-27T11:55:00.597380562Z
...
_fetch_api_fetcher_ref <- include("/tmp/junit2639281907744998091/fetch-api.gs")
...
data <- @rawData
strict <- true
_validation_data_validator_ref <- include("/tmp/junit2639281907744998091/validate-data.gs")
...
inputData <- @validated
format <- @targetFormat
_transform_data_transformer_ref <- include("/tmp/junit2639281907744998091/transform-data.gs")
...
# Stage: statistics
values <- @transformed
_statistics_stats_calculator_ref <- include("/tmp/junit2639281907744998091/calculate-stats.gs")
...
_finalize_finalizer_ref <- include("/tmp/junit2639281907744998091/finalize-output.gs")
