...
public class ContextualBeanSupplier<Bean> implements IContextualBeanSupplier<Bean> {
    ...
    public ContextualBeanSupplier(Optional<String> provider, BeanReference<Bean> query) {
        log.atTrace().log("Entering ContextualBeanSupplier constructor with provider: {} and query: {}", provider, query);
        this.query = Objects.requireNonNull(query, "query cannot be null");
        this.provider = Objects.requireNonNull(provider, "Provider cannot be null");
        log.atDebug().log("ContextualBeanSupplier initialized with query: {} and provider: {}", query, provider);
        log.atTrace().log("Exiting ContextualBeanSupplier constructor");
    }
