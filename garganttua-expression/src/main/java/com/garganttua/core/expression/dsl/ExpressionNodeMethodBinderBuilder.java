...
public class ExpressionNodeMethodBinderBuilder<S>
    ...
    @Override
    public IExpressionMethodBinderBuilder<S> method(Method method, Class<IExpressionNode<S, ISupplier<S>>> returnType, Class<?>... parameterTypes) throws DslException {
        log.atDebug().log("Checking if method {} is static", method.getName());
        if (!Methods.isStatic(method)) {
            throw new DslException("Method " + method.getName() + " must be static for expression binding");
        }
    ...
    @Override
    public IExpressionMethodBinderBuilder<S> method(ObjectAddress methodAddress, Class<IExpressionNode<S, ISupplier<S>>> returnType, Class<?>... parameterTypes) throws DslException {
        log.atDebug().log("Checking if method at address {} is static", methodAddress);
        if (!Methods.isStatic(this.methodOwner, methodAddress)) {
            throw new DslException("Method at address " + methodAddress + " must be static for expression binding");
        }
    ...
    @Override
    public IExpressionMethodBinderBuilder<S> method(String methodName, Class<IExpressionNode<S, ISupplier<S>>> returnType, Class<?>... parameterTypes) throws DslException {
        // First call super to resolve the method
